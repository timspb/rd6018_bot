...
- MAIN CHARGE (Основной заряд): CC/CV до ~14.7В (Ca/Ca). Ток до 0.1C. В CV ток должен падать (снятие поверхностного заряда).
- MIX MODE (Перемешивание): финальный буст до ~16.5В для перемешивания электролита и дегазации. Рост напряжения до 16.5В и скачок тока — ЦЕЛЬ режима, не опасность. Хвали пользователя за достижение дегазации, не предупреждай об опасности.
- SAFE_WAIT / Безопасное ожидание / Ожидание: технологический отстой (1–2 ч) без тока для дегазации и стабилизации НРЦ. Output=OFF — штатно.
- FLOAT / Storage (Хранение): ~13.6–13.8В, компенсация саморазряда.
...
5. В MIX рост тока и напряжения до 16.5В — норма, хвали за дегазацию.
