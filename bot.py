...
def format_status_message(ctx: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "<b>📊 СТАТИСТИКА</b>",
        f"├ Заряд: <b>{cap:.2f} Ah</b>" if cap is not None else "├ Заряд: <i>н/д</i>",
        f"├ Энергия: <b>{en:.1f} Wh</b>" if en is not None else "├ Энергия: <i>н/д</i>",
        f"└ Мощность: <b>{p:.1f} Вт</b>" if p is not None else "└ Мощность: <i>н/д</i>",
        "",
        "<b>🌡 ТЕМПЕРАТУРА</b>",
        f"├ Блок: <b>{t_int:.1f} °C</b>" if t_int is not None else "├ Блок: <i>нет данных</i>",
        f"└ АКБ: <b>{t_ext:.1f} °C</b>" if t_ext is not None else "└ АКБ: <i>нет данных</i>",
        "",
        "<b>⚙️ УСТАВКИ</b>",
        f"├ Напряжение: <b>{sv:.2f} В</b>" if sv is not None else "├ Напряжение: <i>н/д</i>",
        f"└ Ток: <b>{si:.2f} А</b>" if si is not None else "└ Ток: <i>н/д</i>",
    ]
