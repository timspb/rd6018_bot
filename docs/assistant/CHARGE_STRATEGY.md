...
- Расчёт выполняется при старте этапа и при восстановлении сессии, а затем может мягко пересчитываться по мере изменения `temp_ext`, если уставка заметно ушла от текущей.
