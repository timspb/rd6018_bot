...
**ACCEPTED / NOT IMPLEMENTED.** Adopted Mix is a separate `MIX_ADOPTED` managed authority. It must use the physically validated D061 edge ownership primitive, but it is not the D061 Adopted Manual session. Chemistry/battery identity must be explicitly confirmed before autonomous chemistry authority. Delta evidence begins fresh at adoption and may use only post-adoption observations. Known/declared prior active Mix time contributes to the hard chemistry budget. Successful adopted-Mix Delta + sticky hold ends in verified Output OFF rather than silently entering AUTO SAFE_WAIT/Storage; hard budget expiry remains abnormal `MIX_TIMEOUT -> verified OFF + diagnose`.

## D063 — unknown prior external Mix age cannot receive a fresh full autonomous budget
**ACCEPTED / SAFETY-ONLY OBSERVER PARTIALLY IMPLEMENTED / FULL MANAGED MIX AGE AUTHORITY NOT IMPLEMENTED.** The HANDS_OFF external-Mix observer may use HA Recorder only when it can prove an uninterrupted `OFF -> ON -> ... -> current ON` edge; otherwise prior age remains unknown. Recorder history is context only and never seeds Delta authority. The observer may remain non-autonomous or hold bounded verified-OFF authority after fresh post-confirmation Delta evidence.

If an already-running external Mix was not reliably observed from its OFF->ON edge and the operator cannot declare its prior elapsed active time, the bot must not create a new full Ca20/EFB24/AGM10 autonomous authority window. D062 must require prior elapsed time or remain in a non-autonomous alternative such as HANDS_OFF observation, Manual or explicit OFF.
