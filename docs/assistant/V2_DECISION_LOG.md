...
**ACCEPTED / SOFTWARE IMPLEMENTED AS ADOPTED MANUAL / EXACT ESPHOME BENCH VALIDATION PENDING.** An already-energized operator-programmed RD6018 may become Pb-managed only through an explicit adoption transaction. The first implemented managed authority is deliberately `Adopted Manual`: it grants no AUTO/Mix chemistry transition authority and writes no Output/V/I/OVP/OCP at the adoption point.

The transaction requires explicit saved-battery/chemistry confirmation, fresh coherent managed telemetry, normal raw protection, legal positive V/I/OVP/OCP protection geometry, managed temperature/hardware-configuration envelope, and repeated TOCTOU readback. The target edge must expose the dedicated live-adoption command, the exact accepted 900 s lease TTL, and fresh authoritative raw register-16 `Protection Status Code == 0`. Positive edge ACK requires generation change, armed healthy state, fresh direct Modbus evidence and replenished near-full 900 s TTL; raw register-16 NORMAL remains mandatory on every managed poll after takeover.

The live V/I/OVP/OCP fingerprint becomes component-wise maximum authority and may only ratchet downward. Out-of-band increases, loss of required managed evidence or authority contradiction terminate toward verified OFF. Adopted Manual never re-energizes after Output becomes OFF and never auto-resumes after restart.

The command uncertainty boundary is explicit: failures while all edge work is still read-only leave durable HANDS_OFF and the external Output/settings untouched. Once invocation of the edge command begins, transport/ACK failure cannot prove whether ownership changed and therefore enters verified-OFF containment. `ADOPTION_PENDING` grants no chemistry/setpoint authority and restart may continue only toward verified OFF. Detailed contract: `D061_MANAGED_LIVE_ADOPTION.md`.

## D062 — adopted Mix is neither Manual nor full AUTO Mix
**ACCEPTED / SOFTWARE IMPLEMENTED / EXACT ESPHOME BENCH VALIDATION PENDING.** Adopted Mix is a separate `MIX_ADOPTED` managed authority. It reuses the D061 live-adoption edge ownership primitive, but it is neither the D061 Adopted Manual session nor full AUTO Mix. Chemistry and physical battery identity are explicitly confirmed before takeover. The takeover itself writes no Output/V/I/OVP/OCP; the live fingerprint becomes the maximum managed authority and can only ratchet downward. Delta evidence starts fresh after successful adoption and consumes only new post-adoption source reports; Recorder history never seeds Imin/Vmax or a finish event.

Known/declared prior active Mix time plus post-adoption active time is charged against the chemistry hard budget. A Delta finish hold that begins before the Ca20/EFB24/AGM10 boundary may complete its accepted sticky 2 h after the boundary; otherwise expiry is abnormal `MIX_TIMEOUT -> verified OFF + diagnose`. Successful adopted-Mix completion also ends in verified Output OFF and never enters AUTO SAFE_WAIT/Storage. Output OFF retires the authority and `MIX_ADOPTED` cannot re-energize it. Persisted `ADOPTION_PENDING`, `ACTIVE` or `OFF_PENDING` never resumes HV authority after process restart; startup containment may proceed only toward verified OFF.

The edge command uncertainty boundary is transaction-local. A stale `command_may_have_executed` value from an earlier operation cannot authorize shutdown of a current external HANDS_OFF session. Failures before the current edge command may have executed remain read-only and leave the external Output/settings untouched; only current-command ambiguity enters verified-OFF containment. This is software contract coverage, not a claim that the target ESPHome package has been flashed or physically takeover-tested.

## D063 — unknown prior external Mix age cannot receive a fresh full autonomous budget
**ACCEPTED / IMPLEMENTED FOR SAFETY-ONLY OBSERVER AND MANAGED MIX / PHYSICAL TAKEOVER VALIDATION PENDING.** The HANDS_OFF external-Mix observer and D062 managed takeover may use HA Recorder as prior-age authority only when it proves an uninterrupted `OFF -> ON -> ... -> current ON` edge. Otherwise the prior age is unknown unless the operator explicitly declares elapsed active time. Recorder history is context/age evidence only and never seeds Delta authority.
...
## D067 — intentional D061 live release retires only the exact adopted OFF authority after durable HANDS_OFF
**ACCEPTED / IMPLEMENTED IN SOFTWARE WITH DETERMINISTIC REGRESSION COVERAGE.** D061's normal crash rule remains fail-closed: persisted `ACTIVE`/`ADOPTION_PENDING` becomes `OFF_PENDING` and may continue only toward verified Output OFF. A deliberate D060 release of that already-adopted Manual session is a different ownership transaction. Once the same exact session has durably committed HANDS_OFF, the stale D061 journal must not retain authority to turn the now externally-owned PSU off on a later monitor poll or process restart.

The bridge is a small durable exact-session release-intent journal written before entering the existing D060 transaction. The marker by itself grants no external ownership. If the release fails or the process crashes before durable HANDS_OFF, `PB_MANAGED` remains authoritative, the marker is discarded on recovery, and ordinary D061 OFF containment remains intact. If durable HANDS_OFF and the marker's D061 `session_id` agree, the D061 journal is retired to `INTERRUPTED` without changing Output/V/I/OVP/OCP; that exact pair is also sufficient to complete journal retirement after a crash between the two durable writes.

A D061 state already in `OFF_PENDING` cannot be converted into HANDS_OFF release. Generic HANDS_OFF without the matching marker, a stale/mismatched marker, missing adopted-Manual software authority, or an ownership state other than positively established `PB_MANAGED` cannot create release authority. During the post-commit handoff window, the same exact marker suppresses only the D061 `lost_pb_managed_mode` OFF claim while D060's release transaction is still in progress. An ambiguous edge ACK after the durable HANDS_OFF commit remains external ownership and cannot resurrect D061 OFF authority or silently roll back to PB control. Detailed incident guidance: `../INCIDENT_RUNBOOK.md` INC-008.

## D068 — foreign Output ownership decision precedes Pb-specific runtime authority
**ACCEPTED / IMPLEMENTED IN SOFTWARE / CI PENDING.** A freshly and positively observed RD6018 Output ON while no AUTO/Manual authority exists is an ownership conflict, not automatically a Pb charge session. The already-existing 45 s orphan window is therefore evaluated **before** Pb-specific battery-temperature, battery-voltage plausibility, recipe-program/readback and regulation-mode checks. The window exists only to let the operator choose the already-defined `Adopt`, `HANDS_OFF`, or verified `Output OFF` ownership recovery path; it grants no managed recipe/chemistry authority and never enlarges the 45 s duration.

Host-side evidence during that bounded window is intentionally ownership-neutral and strict: canonical Output freshness metadata must be present/fresh, RD protection evidence must be present/fresh and NORMAL, and a positively observed internal PSU temperature at or above the existing hard limit remains immediate verified-OFF authority. Stale/unknown Output or protection evidence, OVP/OCP/OPP/unknown protection, or expiry of the window are not graced. Missing `temp_int` alone is not used to seize the external program during this short interval because D064's edge-local intrinsic protection remains the independent PSU backstop; the host does not renew the Pb edge lease merely because an unowned external Output was observed.
...
- `cd20e34...`: D061 managed live-adoption software baseline — dedicated edge live-adopt command, exact 900 s TTL proof, authoritative raw register-16 gate and Adopted Manual takeover without Output/setpoint writes.
- `795e7dc...`: D061 code-review hardening — pre-command races remain non-actuating; post-command ambiguity forces verified OFF; managed temperature/hardware and measured working envelopes are enforced before takeover.
- `c47a701...` + `b843f97...` + `e91ac4a...`: D062/D063 authoritative managed-Mix core, Telegram/HMI composition and terminal-panel lifecycle; takeover is no-reprogram, fresh-Delta-only and OFF-terminal.
- `0ad605b...` + `f401f38...`: D062/D063 regression coverage for non-shrinking prior-age authority, transaction-local edge uncertainty and managed-Mix workspace/terminal routing.
