"""Read-only comparison with the legacy safety-wrapper requirements."""
...
class LegacyExecutionPolicyAdapter:
    @staticmethod
    def requirements(intent: SafeOutputIntent) -> dict[str, Any]:
    ...
    @staticmethod
    def compare(v2_requirements: Mapping[str, Any], v3_requirements: Mapping[str, Any]) -> ExecutionParityResult:
        ...
        return ExecutionParityResult("MATCH" if not fields else "MISMATCH", fields, dict(v2_requirements), dict(v3_requirements), "requirements_equal" if not fields else "requirements_differ")
