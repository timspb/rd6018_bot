...
from legacy_recipe_adapter import (
    authorize_legacy_target,
    build_legacy_charge_context,
    chemistry_for_legacy_profile,
)
...
class LegacyRecipeAdapterTests(unittest.TestCase):
    ...
    def test_agm_normal_rejects_legacy_mix_target(self):
    ...
    def test_efb_17_5_requires_conditioning_expert_authorization(self):
        context = build_legacy_charge_context(
            ...
            capacity_ah=70,
            battery_id="efb-1",
            intent=ChargeIntent.CONDITIONING,
            condition=BatteryCondition.REHYDRATED,
        )
        ordinary = authorize_legacy_target(
            context,
            stage="conditioning",
            target_voltage_v=17.5,
            target_current_a=3.0,
            expert_high_voltage=False,
        )
        expert = authorize_legacy_target(
            context,
            stage="conditioning",
            target_voltage_v=17.5,
            target_current_a=3.0,
    ...
    def test_current_ceiling_is_stage_sensitive(self):
        ...
        main = authorize_legacy_target(
            context,
            stage="Main Charge",
            target_voltage_v=14.8,
            target_current_a=7.0,
        )
        hv = authorize_legacy_target(
            context,
            stage="Mix Mode",
            target_voltage_v=16.5,
            target_current_a=4.0,
        )
