...
def analysis_at(ts, *, voltage, current, is_cv=True, is_cc=False, current_min=0.2, seconds_since_min=0.0, events=()):
    metrics = SignalMetrics(
        d_voltage_v_per_min=0.0, d_current_a_per_min=0.0, d_temp_c_per_min=0.0,
        current_min_a=current_min, seconds_since_current_min=seconds_since_min,
        ...
        current_plateau_span_a=0.0, current_plateau_center_a=current,
        reversal_confirmations=0, voltage_max_v=voltage if is_cc else None,
        seconds_since_voltage_max=0.0 if is_cc else None, delta_voltage_from_max_v=0.0 if is_cc else None,
        voltage_reversal_threshold_v=0.03 if is_cc else None, voltage_reversal_confirmations=0,
    )
    return SignalAnalysis(sample=SignalSample(ts, voltage, current, 25.0, is_cv=is_cv, is_cc=is_cc), metrics=metrics, events=frozenset(events))


class FixedRuntime:
    def __init__(self, analysis, decision=RecoveryDecision.CONTINUE):
        self.analysis, self.decision, self.records = analysis, decision, []
    def observe(self, point, *, legacy_actions=None, output_is_on=True):
        result = RecoveryDecisionResult(decision=self.decision, reason=f"fixed_{self.decision.value}", evidence=self.analysis.events)
        record = SimpleNamespace(point=point, analysis=self.analysis, decision=result, legacy_effect="continue", disagreement=None)
...
class V2ProductionControllerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_normal_tail_enters_standard_mix(self):
        now = 10000.0
