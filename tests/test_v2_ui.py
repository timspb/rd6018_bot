...
class V2UiTests(unittest.TestCase):
    def test_normal_preview_preserves_standard_auto_hv_chain(self):
        ...
        self.assertIn("V1-compatible", preview.text)
        self.assertIn("Mix: до <b>16.3 V</b>", preview.text)
        self.assertIn("Ограничение профиля: <b>16.30 V</b>", preview.text)

    def test_diagnostic_preview_explicitly_forbids_high_voltage_stage(self):
        preview = build_program_preview(
            profile="AGM", capacity_ah=70, intent=ChargeIntent.DIAGNOSTIC,
            condition=BatteryCondition.HEALTHY,
        )
    ...
    def test_recovery_preview_exposes_hv_and_mode_specific_finish(self):
        preview = build_program_preview(
            profile="EFB", capacity_ah=80, intent=ChargeIntent.RECOVERY,
            condition=BatteryCondition.SULFATED_SUSPECTED,
        )
        self.assertIn("Высоковольтный этап", preview.text)
        self.assertIn("16.5 V", preview.text)
        self.assertIn("24 ч", preview.text)
    ...
    def test_cv_detail_uses_current_evidence_only(self):
        text = format_active_evidence({
            ...
            "decision": "continue", "finish_hold_started_at": None,
            "metrics": {"current_min_a": 0.41, "delta_current_from_min_a": 0.07,
                        "reversal_threshold_a": 0.123, "seconds_since_current_min": 3600,
                        "voltage_max_v": 16.5, "delta_voltage_from_max_v": 0.05,
    ...
    def test_cc_detail_uses_voltage_evidence_only(self):
        text = format_active_evidence({
            ...
            "decision": "finish_stage", "finish_hold_started_at": 1.0,
            "metrics": {"voltage_max_v": 16.47, "delta_voltage_from_max_v": 0.05,
                        "voltage_reversal_threshold_v": 0.03, "seconds_since_voltage_max": 900,
                        "current_min_a": 0.1, "delta_current_from_min_a": 0.2,
    ...
    def test_battery_card_surfaces_longitudinal_state_without_dev_labels(self):
        lifecycle = BatteryLifecycle(
            ...
            cycles_since_refill=2, measured_capacity_ah=61, cca_a=610,
            internal_resistance_mohm=6.3,
        )
        record = BatteryRecord(
            identity=BatteryIdentity("varta-70", BatteryChemistry.AGM, 70, manufacturer="Varta", model="AGM 70"),
            lifecycle=lifecycle,
        )
