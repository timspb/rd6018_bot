...
from runtime.output.bridge import (
    HardwareCapability, HardwareSnapshot, LegacyHardwareAdapter,
    PhysicalBridgeAdapter, validate_readback,
)
...
class V3PhysicalBridgeDiscoveryTests(unittest.TestCase):
    def test_capability_snapshot_and_limits(self):
        ...
        mapped = LegacyHardwareAdapter.capability_from_mapping(capability.__dict__)
    ...
    def test_missing_snapshot_fields_are_not_commands(self):
        snapshot = LegacyHardwareAdapter.snapshot_from_mapping({"timestamp": 1.0, "connection_state": "missing"})
